...
bestIter_test <- gbm.perf(object = fit, method = "cv")
summary(fit)
```
...
pred <- predict(fit, test_x, bestIter_test)
pred_mat <- matrix(0, nrow = dim(test_x)[1], ncol = 1)
for (i in seq(1, dim(test_x)[1], 1)) {
  beg <- (i - 1) * 9 + 1
  end <- i * 9
  k <- which.max(pred[beg:end])
  pred_mat[i, 1] <- k
}

# Compare misclassifcations to test data
test_y <- as.matrix(test_y)
test_y <- as.matrix(map(test_y, as.integer))

count_miss <- 0
for (i in seq_len(length(test_y))) {
  if (test_y[i] != pred_mat[i, 1]) {
